...
description: "The Madder Controller State for deserialization and/or the [Unity Input System](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.0/manual/index.html)"
icon: "dice"
---

## Madder Controller State

### Overview

The Madder controller state is a struct used for deserializing the input sent from a player's Madder controller app to the Unity game. The WebGL build calls [UpdateMadderControllerState()](madder-manager#updatemaddercontrollerstate) on the Madder Manager to update the Madder controller state. The inputs from the player's controller are sent as follows:
...
This class, along with the [Madder Controller](madder-controller) class, defines the layout of the Madder Controller for the Unity Input system.
...
If you are not using the Unity Input System, you may receive this information directly and interact with it as you need by listening to the [OnUpdateMadderControllerState](madder-manager#onupdatemaddercontrollerstate) event.
